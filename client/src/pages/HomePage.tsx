...
export function HomePage({ isAuthenticated }: HomePageProps) {
      ...
      if (selectedAnime) {
        ...
        setContent((prev) => ({
          ...
          anime: prev.anime.map((item) =>
            item.id === id ? ({ ...item, ...formData } as Anime) : item,
          ),
        }));
  ...
  const handleDeleteAnime = async (id: number) => {
    try {
      ...
      setContent((prev) => ({
        ...prev,
        anime: prev.anime.filter((item) => item.id !== id),
      }));
    } catch (err) {
  ...
  const handleDeleteFilm = async (id: number) => {
    try {
      ...
      setContent((prev) => ({
        ...prev,
        films: prev.films.filter((item) => item.id !== id),
      }));
    } catch (err) {
      console.error("Error:", err);
    }
  ...
  const handleDeleteSeries = async (id: number) => {
    try {
      ...
      setContent((prev) => ({
        ...prev,
        series: prev.series.filter((item) => item.id !== id),
      }));
    } catch (err) {
      console.error("Error:", err);
    }
    ...
    <>
      <main className={styles.page}>
        ...
        <p className={styles.pageSubtitle}>
          Ласкаво просимо! Тут зібрані найкращі аніме, фільми та серіали які
          дивився hishiya.
        </p>
                  ...
                  {content.anime.map((item) => (
                    <li key={`anime-${item.id}`} className={styles.listItem}>
                      <AnimeCard
                        anime={item}
                        onDelete={
                          isAuthenticated ? handleDeleteAnime : undefined
                        }
                        onEdit={isAuthenticated ? handleEditAnime : undefined}
                      />
                    </li>
                  ))}
                </ul>
                  ...
                  {content.films.map((item) => (
                    <li key={`film-${item.id}`} className={styles.listItem}>
                      <FilmCard
                        film={item}
                        onDelete={
                          isAuthenticated ? handleDeleteFilm : undefined
                        }
                        onEdit={isAuthenticated ? handleEditFilm : undefined}
                      />
                    </li>
                  ))}
                </ul>
                  ...
                  {content.series.map((item) => (
                    <li key={`series-${item.id}`} className={styles.listItem}>
                      <SeriesCard
                        series={item}
                        onDelete={
                          isAuthenticated ? handleDeleteSeries : undefined
                        }
                        onEdit={isAuthenticated ? handleEditSeries : undefined}
                      />
                    </li>
                  ))}
                </ul>
