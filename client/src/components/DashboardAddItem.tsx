...
import { addFilm, addSeries, addAnime, type DashboardItem } from '../api/dashboardApi';
...
const DashboardAddItem: React.FC = () => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        setItem(prev => ({
            ...prev,
            [name]: value === '' ? null : name === 'rating' ? parseFloat(value) : value,
        }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let response;
            if (itemType === 'film') {
                response = await addFilm(formData);
            } else if (itemType === 'series') {
                response = await addSeries(formData);
            } else {
                response = await addAnime(formData);
            }
            setMessage(`Successfully added ${itemType}: ${item.title}`);
            setItem({ title: '', description: null, release_date: null, rating: null });
            setPosterFile(null); 
        } catch (error) {
    ...
    return (
        <div className={styles.container}>
            ...
            <form onSubmit={handleSubmit} className={styles.form}>
                <select value={itemType} onChange={handleTypeChange} className={styles.select}>
